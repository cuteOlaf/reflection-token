...
contract DaftCoin is Context, IBEP20, Ownable {
    ...
    uint256 private constant _MAX = ~uint256(0);                                    // MAX = 2 ^ 256 - 1
    uint256 private constant _DECIMALFACTOR = 10 ** uint256(_DECIMALS);             // DECIMALFACTOR = 10 ^ 8
    uint256 private constant _GRANULARITY = 100;                                    // 
    
    uint256 private _tTotal = 100000000 * _DECIMALFACTOR;                           // _tTotal = 10 ^ 16
    uint256 private _rTotal = (_MAX - (_MAX % _tTotal));                            // _rTotal = MAX/_tTotal * _tTotal, a multiple of _tTotal
    
    uint256 private _tFeeTotal;                                                     // total fee
    uint256 private _tBurnTotal;                                                    // total burn
    
    uint256 private      _TAX_FEE = 0;                                              // tax_fee
    uint256 private     _BURN_FEE = 0;                                              // burn_fee
    uint256 private constant _MAX_TX_SIZE = 100000000 * _DECIMALFACTOR;             // maximum transaction size 10 ^ 8

    constructor () public {
        _rOwned[_msgSender()] = _rTotal;                                            // number of tokens left to the manager
